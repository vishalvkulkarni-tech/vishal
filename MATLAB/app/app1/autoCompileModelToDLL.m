...
%   - MEX compiler configured (run 'mex -setup' first time)
    ...
    % Force rebuild by updating model (touch it)
    set_param(modelName, 'Description', ['Last compiled: ' datestr(now)]);
    save_system(modelName);
    
    try
        % Generate code using Simulink Coder command
        evalc('rtwbuild(modelName)');  % Use rtwbuild for direct code generation
    ...
    % Code is generated to: ModelName_ert_rtw (in current directory)
    codeDir = fullfile(pwd, [modelName '_ert_rtw']);
    
    if ~exist(codeDir, 'dir')
        error('Code generation directory not found: %s\nCode generation may have failed.', codeDir);
    end
    ...
    %% Step 5: Create model wrapper MATLAB function
    if verbose, fprintf('\nStep 5: Creating MATLAB wrapper...\n'); end
    
    % Instead of MEX, create a MATLAB function wrapper
    wrapperMFile = fullfile(outputDir, [modelName '_run.m']);
    fid = fopen(wrapperMFile, 'w');
    fprintf(fid, 'function outputs = %s_run(inputs)\n', modelName);
    fprintf(fid, '%% Auto-generated wrapper for %s\n', modelName);
    fprintf(fid, '%% This function calls the model using sim()\n\n');
    fprintf(fid, '    %% Set model inputs\n');
    ...
    if verbose, fprintf('   ✓ MATLAB wrapper created: %s\n', wrapperMFile); end
    
    % Copy generated code files to output directory
    copyfile(mainHFile, outputDir);
    copyfile(mainCFile, outputDir);
    
    if verbose
        fprintf('   ✓ Code files copied to: %s\n', outputDir);
    end
    
    %% Return paths
    dllPath = wrapperMFile;  % Return the MATLAB wrapper
    ...
    % Build MEX command - MEX doesn't use -shared, just compile all sources
    mexCmd = {};
    if verbose
        mexCmd{end+1} = '-v';  % Verbose
    end
