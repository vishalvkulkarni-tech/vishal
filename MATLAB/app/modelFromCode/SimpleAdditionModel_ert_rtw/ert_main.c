...
#include "SimpleAdditionModel.h"       /* Model header file */
...
void rt_OneStep(void)
{
  ...
  /* Check for overrun */
  if (OverrunFlag) {
    rtmSetErrorStatus(SimpleAdditionModel_M, "Overrun");
    return;
  }
...
int_T main(int_T argc, const char *argv[])
{
  ...
  fflush((NULL));
  while (rtmGetErrorStatus(SimpleAdditionModel_M) == (NULL)) {
    /*  Perform application tasks here */
  }

  /* Terminate model */
  SimpleAdditionModel_terminate();
  return 0;
}
